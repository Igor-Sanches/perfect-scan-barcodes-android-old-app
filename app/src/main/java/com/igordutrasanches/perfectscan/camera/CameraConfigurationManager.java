...
import com.igordutrasanches.perfectscan.camera.open.CameraFacing;
...
final class CameraConfigurationManager {
  ...
  void setDesiredCameraParameters(OpenCamera camera, boolean safeMode) {
    ...
    CameraConfigurationUtils.setFocus(
        parameters,
        Key.getAutoFocus(context),
        Key.getDisableContinuesFocus(context),
        safeMode);

    if (!safeMode) {
      if (Key.getDisableInvert(context)) {
        CameraConfigurationUtils.setInvertColor(parameters);
      }

      if (!Key.getDisableBarcodeEnceneMode(context)) {
        CameraConfigurationUtils.setBarcodeSceneMode(parameters);
      }

      if (!Key.getDisableMetering(context)) {
  ...
  private void doSetTorch(Camera.Parameters parameters, boolean newSetting, boolean safeMode) {
    ...
    if (!safeMode && !Key.getDisableExposure(context)) {
